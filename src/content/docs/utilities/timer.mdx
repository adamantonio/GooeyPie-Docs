...
While there are `app.set_interval()` and `app.set_timeout()` methods for [scheduling functions](/windows/the-main-window/#timeouts-and-intervals) to run after a certain amount of time, the Timer class is more suitable for applications that require precise timekeeping, such as stopwatches, countdowns, or any situation where you need to track how much time has passed.
